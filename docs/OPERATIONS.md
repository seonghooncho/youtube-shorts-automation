...
성과 수집까지 사용하려면 refresh token에 `youtube.readonly`와 `yt-analytics.readonly` scope가 포함되어야 합니다. scope가 부족하면 metrics Lambda는 콘텐츠 레코드에 `METRICS_BLOCKED`와 오류 사유를 남깁니다. Analytics API는 데이터 지연이 있을 수 있으므로 행이 없으면 실패가 아니라 `METRICS_PENDING`으로 기록합니다.
...
SSM에 저장된 refresh token이 `youtube.upload` scope만 갖고 있으면 `videos.list`, `processingDetails`, `videos.delete`, Analytics 조회가 `ACCESS_TOKEN_SCOPE_INSUFFICIENT`로 거부됩니다. 이 scope로는 업로드는 가능하지만 기존 업로드의 처리 상태 확인, 삭제, 성과 수집은 API로 수행할 수 없습니다.
