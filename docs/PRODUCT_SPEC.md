...
4. 기존 publish-ready 재고를 확인하고 부족분에 예약일 부여
5. AWS Polly로 음성 및 speech marks 생성
6. speech marks를 SRT 자막으로 변환
7. Pixabay 배경 영상을 세로형으로 병합
8. MoviePy/FFmpeg로 최종 쇼츠 렌더링
9. S3 `publish-ready/`와 DynamoDB에 업로드 대기 상태 저장
10. daily upload workflow가 예약 시간이 지난 영상을 YouTube에 업로드
